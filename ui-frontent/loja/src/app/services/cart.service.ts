...
export class CartService {
  ...
  computeCartTotals() {
    ...
    // publish the new values price ... all the subscribers will receive the new data
    this.totalPrice.next(totalPriceValue);
    this.totalQuantity.next(totalQuantityValue);

    // log cart data just for debbugin purposes
  ...
  remove(theCartItem: CartItem) {
    ...
    // if found, remove it from the array at the given item
    if (itemIndex > -1) {
