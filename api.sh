...
# activity pdf
admin_activity_export_pdf () {
  if [ -z "$TOKEN" ]; then
    echo "TOKEN is empty. Run login_admin first."
    return 1
  fi

  curl -sOJ "$API/api/admin/activity/export/pdf" \
  -H "Authorization: Bearer $TOKEN"
}
...
admin_customers() {
  ...
  curl -s "$API/api/admin/customers?page=$page&size=$size" \
  -H "Authorization: Bearer $TOKEN"  | jq
}


admin_dashboard_daily_order_per_day() {
  if [ -z "$TOKEN" ]; then
    echo "TOKEN is empty. Run login_admin first."
    return 1
  fi 

  curl -s "$API/api/admin/dashboard/orders-per-day" \
  -H "Authorization: Bearer $TOKEN"  | jq
}

admin_dashboard_daily_revenue_per_day() {
  if [ -z "$TOKEN" ]; then
    echo "TOKEN is empty. Run login_admin first."
    return 1
  fi 

  curl -s "$API/api/admin/dashboard/revenue-per-day" \
  -H "Authorization: Bearer $TOKEN"  | jq
}
...
admin_dashboard_top_products() {
  ...
  curl -s "$API/api/admin/dashboard/top-products" \
    -H "Authorization: Bearer $TOKEN" | jq
}

admin_dashboard_products_by_stock() {
  if [ -z "$TOKEN" ]; then
    echo "TOKEN is empty. Run login_admin first."
    return 1
  fi

  curl -s "$API/api/admin/dashboard/products-by-stock" \
    -H "Authorization: Bearer $TOKEN" | jq
}
